...
function Row(props) {
  ...
  // // Calculate the current time + 2 hours
  // const twoHoursLater = new Date();
  // twoHoursLater.setHours(twoHoursLater.getHours() + 2);

  // // Check if the end time is not available and start time is within 2 hours
  // const shouldHighlight =
  //   !row.endTime && new Date(row.startTime) < twoHoursLater;
                ...
                <div>
                  {/* <Typography variant="p" gutterBottom component="div">
                    ...
                    <span
                      style={{
                        color:
                          moment(row.startTime, "hh:mm A").isValid() &&
                          currentTime.isAfter(
...
function PaddingCompaintTable() {
  ...
  let getData = async () => {
    let res = await axios.post(
      "https://spr-cms-babe93641764.herokuapp.com/padding-complaint/complaint_data_padding",
      {
    ...
    setLoader(false);
    setCountData(res.data.paddingComplanitCount);
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://spr-cms-babe93641764.herokuapp.com/padding-complaint/search_complaint_padding",
      {
        search: values,
      }
    );
...
export default PaddingCompaintTable;
