...
//   return <div>CompleteComplaint</div>;
// }

// export default CompleteComplaint;
...
import Complaint from "./ReviewComplainsTable";
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{row.isCompleted}</TableCell>
        <TableCell align="center">
          <div className="d-flex flex-row justify-content-center gap-2">
            ...
            {row.createDateAt === moment(new Date()).format("YYYY-MM-DD") ? (
              <Button
                variant="danger"
...
export default function CompleteComplaint() {
  ...
  let getData = async () => {
    let data = await axios.post(
      "https://spr-cms-babe93641764.herokuapp.com/review-complaint/complete_complaint",
      {
  ...
  let handleSubmit = async (values) => {
    ...
    if (response.data.statusCode === 200) {
      setModalShowForPopupForm(false);
      getData();
      // swal("", "Complaint Repeat Successfully", "success");
      swal({
        title: "",
        text: "Complaint Repeat Successfully",
        icon: "success",
      }).then(() => {
